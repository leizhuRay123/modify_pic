...
CONTAINS
...
#ifdef PHOTONS
    nvar = nvar+1
    nvar = nvar + 1 ! for generating_time
    nvar = nvar + 1 ! for cs_times
    nvar = nvar + 3  !for stokes
    nvar = nvar + 3  !for stokes direction
#endif
#if defined(PHOTONS) || defined(BREMSSTRAHLUNG)
    nvar = nvar+1
#endif
  ...
  SUBROUTINE pack_particle(array, a_particle)
    ...
    cpos = cpos+1
    array(cpos) = a_particle%generate_time
    cpos = cpos+1
    array(cpos) = a_particle%cs_times
    cpos = cpos+1
    array(cpos:cpos+2) = a_particle%stokes
    cpos = cpos+3
    array(cpos:cpos+2) = a_particle%stokes_dir
    cpos = cpos+3
#endif
#if defined(PHOTONS) || defined(BREMSSTRAHLUNG)
  ...
  SUBROUTINE unpack_particle(array, a_particle)
    ...
    cpos = cpos+1
    a_particle%generate_time = array(cpos)
    cpos = cpos+1
    a_particle%cs_times = array(cpos)
    cpos = cpos+1
    a_particle%stokes = array(cpos:cpos + 2)
    cpos = cpos+3
    a_particle%stokes_dir = array(cpos:cpos + 2)
    cpos = cpos+3
#endif
#if defined(PHOTONS) || defined(BREMSSTRAHLUNG)
  ...
  SUBROUTINE init_particle(new_particle)
    ...
    new_particle%generate_time = 0.0_num
    new_particle%cs_times = 0.0_num
    new_particle%stokes = 0.0_num
    new_particle%stokes_dir = 0.0_num
