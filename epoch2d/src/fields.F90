...
CONTAINS
  ...
  SUBROUTINE calc_time_profile(values,time,phase,omega,t0,t1,tw)
    !For calculate the time_profile values of the external fields or current
    !Xiey
      REAL(num), INTENT(IN) :: time, phase,omega,t0,t1,tw
      REAL(num), INTENT(OUT) :: values
      !perturbation begin from t0 end at t1 in a sin formation  
      IF ((time .GE. t0) .AND. (time .LE. t1)) THEN
        values = SIN(omega*(time-t0) + phase)
        !values = 1.0_num
        
      ELSE
        values = 0.0_num
      END IF
      !IF (rank == 10) WRITE(*,*) values
      
      !decay formation
      !values = SIN(omega*(time-t0) + phase)*EXP(-(time-t1)**2/(tw)**2)
!      IF (time .GE. sta_cur_time0) THEN
!        values = SIN(sta_cur_omega*time)*EXP(-(time-sta_cur_time0)**2/(sta_cur_timew)**2)
!        !values = EXP(-(time - sta_cur_time0)**2/(sta_cur_timew)**2)
!      ELSE
!        values = 0.0_num 
!!      WRITE(*,*) sta_cur_time0 
!!      WRITE(*,*) sta_cur_timew
!      END IF
  ...
  SUBROUTINE update_eb_fields_half
      REAL(num) :: values_jx =  0.0_num
      REAL(num) :: values_jy =  0.0_num
      REAL(num) :: values_jz =  0.0_num
    ...
    !Xiey
    !CALL calc_time_profile(values_jx,time,0);
    CALL calc_time_profile(values_jz,time,sta_cur_phase,omega=sta_cur_omega,t0=sta_cur_time0,t1=sta_cur_time1,tw = sta_cur_timew);
    jx(:,:) = jx(:,:) + jx0(:,:) + values_jx*jx1(:,:)
    jy(:,:) = jy(:,:) + jy0(:,:) + values_jy*jy1(:,:)
    jz(:,:) = jz(:,:) + jz0(:,:) + values_jz*jz1(:,:)
    ...
    jy(:,:) = jy(:,:) - (jy0(:,:) + values_jy*jy1(:,:))
    jz(:,:) = jz(:,:) - (jz0(:,:) + values_jz*jz1(:,:))
    !jx(:,:) = jx(:,:) - jx0(:,:)*(1 + delta_jx0*values_jx)
    !jy(:,:) = jy(:,:) - jy0(:,:)*(1 + delta_jy0*values_jy)
    !jz(:,:) = jz(:,:) - jz0(:,:)*(1 + delta_jz0*values_jz)
    !jx(:,:) = jx(:,:) - jx0(:,:)*values_jx
    !jy(:,:) = jy(:,:) - jy0(:,:)*values_jy
    !jz(:,:) = jz(:,:) - jz0(:,:)*values_jz
  ...
  SUBROUTINE update_eb_fields_final
    ...
    !CALL calc_time_profile(values,time,REAL(0.0,num));
    CALL calc_time_profile(values_jz,time,sta_cur_phase,omega=sta_cur_omega,t0=sta_cur_time0,t1=sta_cur_time1,tw = sta_cur_timew);
    !WRITE(*,*) values_jz,sta_cur_phase,sta_cur_omega
    !WRITE(*,*) time, sta_cur_time0,sta_cur_time1
