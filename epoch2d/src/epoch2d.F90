...
PROGRAM pic
  ...
  ! manual_load_wein_for photon
  IF (wein_temperature > 0.0_num) THEN
  CALL manual_load_wein
  ELSEIF(delta_f_temp > 0.0_num) THEN
  CALL manual_load_delta_f
  ENDIF
  ...
  DO
    ...
    IF (push) THEN
      ...
      CALL push_particles
      IF (use_particle_lists .OR. (use_compton_scatter .AND. time > cp_start_time) &
          .OR. merging) THEN
        ...
        IF (merging) THEN
            ...
            DO ispecies = 1, n_species
                PRINT*,'Part Num:',ispecies,species_list(ispecies)%global_count
            ENDDO
            ENDIF
        END IF
        !xuz_merge
        ...
        !for compton scatter
        IF (use_compton_scatter .AND. time > cp_start_time) THEN
            !WRITE(*,*) species_list(1)%secondary_list(1,1)%count
  ...
  !XY I need to release the table after output_routines because I have diagonise in optical detection.
#ifdef PHOTONS
