  ...
  SUBROUTINE output_routines(step, force_write)   ! step = step index
    ...
    DO iprefix = 1,SIZE(file_prefixes)
      ...
      CALL write_field(c_dump_ez, code, 'ez', 'Electric Field/Ez', 'V/m', &
          c_stagger_ez, ez)
      !For calcualte laser stokes
      CALL write_field(c_dump_ey2, code, 'ey2', 'Electric Field/Ey2', '(V/m)^2', &
          c_stagger_ey, ey**2)
      CALL write_field(c_dump_ez2, code, 'ez2', 'Electric Field/Ez2', '(V/m)^2', &
          c_stagger_ez, ez**2)
      CALL write_field(c_dump_eyez, code, 'eyez', 'Electric Field/EyEz', '(V/m)^2', &
          c_stagger_ez, ey*ez)
      !
      ...
      CALL write_field(c_dump_jx0, code, 'jx0', 'Current/Jx0', 'A/m^2', &
          c_stagger_jx, jx0)
      CALL write_field(c_dump_jy0, code, 'jy0', 'Current/Jy0', 'A/m^2', &
          c_stagger_jy, jy0)
      CALL write_field(c_dump_jz0, code, 'jz0', 'Current/Jz0', 'A/m^2', &
          c_stagger_jz, jz0)
      ...
      DO isubset = 1, n_subsets + 1
        ...
        mask = iomask(c_dump_total_optical_intensity_sum)
        IF(IAND(mask,code) /= 0) THEN
            CALL calc_total_optical_intensity

            CALL sdf_write_srl(sdf_handle, 'total_optical_I1', &
                  'Total Optical Intensity I1 in Simulation', &
                  total_optical_I1)
            CALL sdf_write_srl(sdf_handle, 'total_optical_I2', &
                  'Total Optical Intensity I2 in Simulation', &
                   total_optical_I2)
            CALL sdf_write_srl(sdf_handle, 'total_optical_U', &
                  'Total Optical Intensity U in Simulation', &
                   total_optical_U)
        ENDIF
        ...
        !XY for stokes
        CALL write_particle_variable(c_dump_part_stokes_I1, code, &
            'Stokes_I1', '', it_output_real)
        CALL write_particle_variable(c_dump_part_stokes_I2, code, &
            'Stokes_I2', '', it_output_real)
        CALL write_particle_variable(c_dump_part_stokes_V, code, &
            'Stokes_V', '', it_output_real)

        CALL write_particle_variable(c_dump_part_stokes_dirx, code, &
            'Dir_I1_x', '', it_output_real)
        CALL write_particle_variable(c_dump_part_stokes_diry, code, &
            'Dir_I1_y', '', it_output_real)
        CALL write_particle_variable(c_dump_part_stokes_dirz, code, &
            'Dir_I1_z', '', it_output_real)

        CALL write_particle_variable(c_dump_part_generate_time, code, &
            'Generate_time', '', it_output_real)
        CALL write_particle_variable(c_dump_part_cs_times, code, &
            'Compton_Scatter_Times', '', it_output_real)
#endif
        ...
        !for polarization
        CALL write_nspecies_field(c_dump_stokes_I, code, &
            'stokes_I', 'Stokes I', 'W/m^2/sr/Hz', &
            c_stagger_cell_centre, calc_polarization, array, (/c_dir_stokes_I/))

        CALL write_nspecies_field(c_dump_stokes_U, code, &
            'stokes_U', 'Stokes U', 'W/m^2/sr/Hz', &
            c_stagger_cell_centre, calc_polarization, array,(/c_dir_stokes_U/))

        CALL write_nspecies_field(c_dump_stokes_Q, code, &
            'stokes_Q', 'Stokes Q', 'W/m^2/sr/Hz', &
            c_stagger_cell_centre, calc_polarization, array,(/c_dir_stokes_Q/))

        CALL write_nspecies_field(c_dump_stokes_N, code, &
            'detect_N', 'Detect N', '', &
            c_stagger_cell_centre, calc_polarization, array,(/c_dir_stokes_N/))

        !for optical polarization
        CALL write_nspecies_field(c_dump_optical_stokes_I, code, &
            'optical_stokes_I', 'Optical Stokes I', 'W/m^2/sr/Hz', &
            c_stagger_cell_centre, calc_optical_polarization, array, (/c_dir_stokes_I/))

        CALL write_nspecies_field(c_dump_optical_stokes_U, code, &
            'optical_stokes_U', 'Optical Stokes U', 'W/m^2/sr/Hz', &
            c_stagger_cell_centre, calc_optical_polarization, array, (/c_dir_stokes_U/))

        CALL write_nspecies_field(c_dump_optical_stokes_Q, code, &
            'optical_stokes_Q', 'Optical Stokes Q', 'W/m^2/sr/Hz', &
            c_stagger_cell_centre, calc_optical_polarization, array, (/c_dir_stokes_Q/))

        CALL write_nspecies_field(c_dump_optical_stokes_V, code, &
            'optical_stokes_V', 'Optical Stokes V', 'W/m^2/sr/Hz', &
            c_stagger_cell_centre, calc_optical_polarization, array, (/c_dir_stokes_V/))

        CALL write_nspecies_field(c_dump_optical_stokes_N, code, &
            'optical_stokes_N', 'Optical Stokes N', '', &
            c_stagger_cell_centre, calc_optical_polarization, array, (/c_dir_stokes_N/))

        CALL write_nspecies_field(c_dump_i1_detect, code, &
            'i1_specific_intensity', 'I1 Specific Intensity', 'W/m^2/sr/Hz', &
            c_stagger_cell_centre, calc_polar_intensity, array, (/c_dir_polar1/))

        CALL write_nspecies_field(c_dump_i2_detect, code, &
            'i2_specific_intensity', 'I2 Specific Intensity', 'W/m^2/sr/Hz', &
            c_stagger_cell_centre, calc_polar_intensity, array, (/c_dir_polar2/))
        ...
        !XY for static current
        CALL write_nspecies_field(c_dump_jx0, code, &
            'jx', 'Jx', 'A/m^2', &
            c_stagger_cell_centre, calc_per_species_current, array, (/c_dir_x/))

        CALL write_nspecies_field(c_dump_jy0, code, &
            'jy', 'Jy', 'A/m^2', &
            c_stagger_cell_centre, calc_per_species_current, array, (/c_dir_y/))

        CALL write_nspecies_field(c_dump_jz0, code, &
            'jz', 'Jz', 'A/m^2', &
            c_stagger_cell_centre, calc_per_species_current, array, (/c_dir_z/))


        CALL write_nspecies_field(c_dump_rel_ux, code, &
            'four_velocity_ux', 'Four_Velocity_Ux', 'm/s', &
            c_stagger_cell_centre, calc_per_species_rel_velocity, array, (/c_dir_x/))

        CALL write_nspecies_field(c_dump_rel_uy, code, &
            'four_velocity_uy', 'Four_Velocity_Uy', 'm/s', &
            c_stagger_cell_centre, calc_per_species_rel_velocity, array,(/c_dir_y/))
        CALL write_nspecies_field(c_dump_rel_uz, code, &
            'four_velocity_uz', 'Four_Velocity_Uz', 'm/s', &
            c_stagger_cell_centre, calc_per_species_rel_velocity, array, (/c_dir_z/))

!        CALL write_nspecies_field(c_dump_ux, code, &
!            'velocity_ux', 'Velocity_Ux', 'm/s', &
!            c_stagger_cell_centre, calc_per_species_ux, array)
!
!        CALL write_nspecies_field(c_dump_uy, code, &
!            'velocity_uy', 'Velocity_Uy', 'm/s', &
!            c_stagger_cell_centre, calc_per_species_uy, array)
!
!        CALL write_nspecies_field(c_dump_uz, code, &
!            'velocity_uz', 'Velocity_Uz', 'm/s', &
!            c_stagger_cell_centre, calc_per_species_uz, array)
!
        CALL write_nspecies_field(c_dump_Pxx, code, &
            'pressure_pxx', 'Pressure_Pxx', 'kg/m/s^2', &
            c_stagger_cell_centre, calc_per_species_Pxx, array)

        
        CALL write_nspecies_field(c_dump_Pxy, code, &
            'pressure_pxy', 'Pressure_Pxy', 'kg/m/s^2', &
            c_stagger_cell_centre, calc_per_species_Pxy, array)
        
        CALL write_nspecies_field(c_dump_Pxz, code, &
            'pressure_pxz', 'Pressure_Pxz', 'kg/m/s^2', &
            c_stagger_cell_centre, calc_per_species_Pxz, array)
        
        CALL write_nspecies_field(c_dump_Pyy, code, &
            'pressure_pyy', 'Pressure_Pyy', 'kg/m/s^2', &
            c_stagger_cell_centre, calc_per_species_Pyy, array)
        
        CALL write_nspecies_field(c_dump_Pyz, code, &
            'pressure_pyz', 'Pressure_Pyz', 'kg/m/s^2', &
            c_stagger_cell_centre, calc_per_species_Pyz, array)
        
        CALL write_nspecies_field(c_dump_Pzz, code, &
            'pressure_pzz', 'Pressure_Pzz', 'kg/m/s^2', &
            c_stagger_cell_centre, calc_per_species_Pzz, array)
        
        CALL write_nspecies_field(c_dump_Hall_Ex, code, &
            'hall_ex', 'Hall_Ex', 'V/m', &
            c_stagger_cell_centre, calc_per_species_Hall_electric_field,array,(/c_dir_x/))
        
        CALL write_nspecies_field(c_dump_Hall_Ey, code, &
            'hall_ey', 'Hall_Ey', 'V/m', &
            c_stagger_cell_centre, calc_per_species_Hall_electric_field, array,(/c_dir_y/))
        
        CALL write_nspecies_field(c_dump_Hall_Ez, code, &
            'hall_ez', 'Hall_Ez', 'V/m', &
            c_stagger_cell_centre, calc_per_species_Hall_electric_field,array,(/c_dir_z/))
        
        CALL write_nspecies_field(c_dump_convection_Ex, code, &
            'convection_ex', 'Convection_Ex', 'V/m', &
            c_stagger_cell_centre, calc_per_species_convection_electric_field, array,(/c_dir_x/))
        
        CALL write_nspecies_field(c_dump_convection_Ey, code, &
            'convection_ey', 'Convection_Ey', 'V/m', &
            c_stagger_cell_centre, calc_per_species_convection_electric_field,array,(/c_dir_y/))
        
        CALL write_nspecies_field(c_dump_convection_Ez, code, &
            'convection_ez', 'Convection_Ez', 'V/m', &
            c_stagger_cell_centre, calc_per_species_convection_electric_field, array,(/c_dir_z/))
        
        CALL write_nspecies_field(c_dump_Nernst_Ex, code, &
            'nernst_ex', 'Nernst_Ex', 'V/m', &
            c_stagger_cell_centre, calc_per_species_Nernst_electric_field, array,(/c_dir_x/))
        
        CALL write_nspecies_field(c_dump_Nernst_Ey, code, &
            'nernst_ey', 'Nernst_Ey', 'V/m', &
            c_stagger_cell_centre, calc_per_species_Nernst_electric_field, array,(/c_dir_y/))
        
        CALL write_nspecies_field(c_dump_Nernst_Ez, code, &
            'nernst_ez', 'Nernst_Ez', 'V/m', &
            c_stagger_cell_centre, calc_per_species_Nernst_electric_field, array,(/c_dir_z/))

        CALL write_nspecies_field(c_dump_Biermann_Ex, code, &
            'nernst_ex', 'Biermann_Ex', 'V/m', &
            c_stagger_cell_centre, calc_per_species_Biermann_electric_field, array,(/c_dir_x/))
        
        CALL write_nspecies_field(c_dump_Biermann_Ey, code, &
            'nernst_ey', 'Biermann_Ey', 'V/m', &
            c_stagger_cell_centre, calc_per_species_Biermann_electric_field, array,(/c_dir_y/))
        
        CALL write_nspecies_field(c_dump_Biermann_Ez, code, &
            'nernst_ez', 'Biermann_Ez', 'V/m', &
            c_stagger_cell_centre, calc_per_species_Biermann_electric_field, array,(/c_dir_z/))


!        CALL write_nspecies_field(c_dump_viscous_xyx_Ey, code, &
!            'viscous_xyx_ey', 'Viscous_xyx_Ey', 'V/m', &
!            c_stagger_cell_centre, calc_viscous_electric_field_xyx, array)
!        
!        CALL write_nspecies_field(c_dump_viscous_xzx_Ez, code, &
!            'viscous_xzx_ez', 'Viscous_xzx_Ez', 'V/m', &
!            c_stagger_cell_centre, calc_viscous_electric_field_xzx, array)
!        
!        CALL write_nspecies_field(c_dump_viscous_xyy_Ex, code, &
!            'viscous_xyy_ex', 'Viscous_xyy_Ex', 'V/m', &
!            c_stagger_cell_centre, calc_viscous_electric_field_xyy, array)
!        
!        CALL write_nspecies_field(c_dump_viscous_yzy_Ez, code, &
!            'viscous_yzy_ez', 'Viscous_yzy_Ez', 'V/m', &
!            c_stagger_cell_centre, calc_viscous_electric_field_yzy, array)
!        
!        CALL write_nspecies_field(c_dump_viscous_xzz_Ex, code, &
!            'viscous_xzz_ex', 'Viscous_xzz_Ex', 'V/m', &
!            c_stagger_cell_centre, calc_viscous_electric_field_xzz, array)
!        
!        CALL write_nspecies_field(c_dump_viscous_yzz_Ey, code, &
!            'viscous_yzz_ey', 'Viscous_yzz_Ey', 'V/m', &
!            c_stagger_cell_centre, calc_viscous_electric_field_yzz, array)
!        
!        CALL write_nspecies_field(c_dump_gyrotropy, code, &
!            'gyrotropy', 'Gyrotropy', '', &
!            c_stagger_cell_centre, calc_per_species_gyrotropy, array)
! 
        ...
        CALL write_nspecies_field(c_dump_rel_temperature_x, code, &
            'rel_temperature_x', 'Rel_Temp_x', 'K', &
            c_stagger_cell_centre, calc_per_species_rel_temperature, array, (/c_dir_x/))
        CALL write_nspecies_field(c_dump_rel_temperature_y, code, &
            'rel_temperature_y', 'Rel_Temp_y', 'K', &
            c_stagger_cell_centre, calc_per_species_rel_temperature, array, (/c_dir_y/))
        CALL write_nspecies_field(c_dump_rel_temperature_y, code, &
            'rel_temperature_z', 'Rel_Temp_x', 'K', &
            c_stagger_cell_centre, calc_per_species_rel_temperature, array, (/c_dir_z/))

        CALL write_nspecies_field(c_dump_magnetic_vector_potential, code, &
            'magnetic_vector_potential', 'Magnetic_Vector_Potential', 'T \cdot m', &
            c_stagger_cell_centre, calc_magnetic_vector_potential, array)
    ...
    END IF
!XY
!    IF (.NOT. use_compton_scatter) THEN
!      DO i = 1, n_species
  ...
  SUBROUTINE average_field(ioutput, avg)
    ...
    IF (avg%dump_single) THEN
      ...
      CASE(c_dump_ez)
        ...
        !
      CASE(c_dump_ey2)
        avg%r4array(:,:,1) = avg%r4array(:,:,1) + REAL(ey**2 * dt, r4)
      CASE(c_dump_ez2)
        avg%r4array(:,:,1) = avg%r4array(:,:,1) + REAL(ez**2 * dt, r4)
      CASE(c_dump_eyez)
        avg%r4array(:,:,1) = avg%r4array(:,:,1) + REAL(ey*ez * dt, r4)
        !
      CASE(c_dump_bx)
      ...
      CASE(c_dump_jx0)
        avg%r4array(:,:,1) = avg%r4array(:,:,1) &
            + REAL(jx0(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
      CASE(c_dump_jy0)
        avg%r4array(:,:,1) = avg%r4array(:,:,1) &
            + REAL(jy0(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
      CASE(c_dump_jz0)
        avg%r4array(:,:,1) = avg%r4array(:,:,1) &
            + REAL(jz0(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
      ...
      CASE(c_dump_stokes_I)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_polarization(array, ispecies-avg%species_sum,c_dir_stokes_I)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)

      CASE(c_dump_stokes_Q)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_polarization(array, ispecies-avg%species_sum,c_dir_stokes_Q)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)

      CASE(c_dump_stokes_U)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_polarization(array, ispecies-avg%species_sum,c_dir_stokes_U)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)

     CASE(c_dump_stokes_N)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_polarization(array, ispecies-avg%species_sum,c_dir_stokes_N)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)
        !for optical polarization
      CASE(c_dump_optical_stokes_I)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_optical_polarization(array, ispecies-avg%species_sum,c_dir_stokes_I)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)

      CASE(c_dump_optical_stokes_Q)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_optical_polarization(array, ispecies-avg%species_sum,c_dir_stokes_Q)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)

      CASE(c_dump_optical_stokes_U)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_optical_polarization(array, ispecies-avg%species_sum,c_dir_stokes_U)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)

      CASE(c_dump_optical_stokes_V)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_optical_polarization(array, ispecies-avg%species_sum,c_dir_stokes_V)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)

      CASE(c_dump_optical_stokes_N)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_optical_polarization(array, ispecies-avg%species_sum,c_dir_stokes_N)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)

!      CASE(c_dump_polar_degree)
!        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
!        DO ispecies = 1, n_species_local
!          CALL calc_polarization_degree(array, ispecies-avg%species_sum)
!          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
!              + REAL(array * dt, r4)
!        END DO
!        DEALLOCATE(array)
!
      CASE(c_dump_i1_detect)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_polar_intensity(array, ispecies-avg%species_sum,c_dir_polar1)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)

      CASE(c_dump_i2_detect)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_polar_intensity(array, ispecies-avg%species_sum,c_dir_polar2)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        END DO
        DEALLOCATE(array)
      ...
      CASE(c_dump_rel_ux)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_rel_velocity(array,ispecies-avg%species_sum,c_dir_x)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_rel_uy)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_rel_velocity(array,ispecies-avg%species_sum,c_dir_y)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_rel_uz)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_rel_velocity(array,ispecies-avg%species_sum,c_dir_z)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      ...
      CASE(c_dump_ux)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_ux(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_uy)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_uy(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_uz)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_uz(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Pxx)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Pxx(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      ...
      CASE(c_dump_Pxy)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Pxy(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Pxz)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Pxz(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Pyy)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Pyy(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Pyz)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Pyz(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Pzz)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Pzz(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      ...
      CASE(c_dump_Hall_Ex)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Hall_electric_field(array, ispecies-avg%species_sum,c_dir_x)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Hall_Ey)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Hall_electric_field(array,ispecies-avg%species_sum,c_dir_y)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Hall_Ez)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Hall_electric_field(array, ispecies-avg%species_sum,c_dir_z)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)

      CASE(c_dump_convection_Ex)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_convection_electric_field(array, ispecies-avg%species_sum,c_dir_x)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_convection_Ey)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_convection_electric_field(array,ispecies-avg%species_sum,c_dir_y)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_convection_Ez)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_convection_electric_field(array, ispecies-avg%species_sum,c_dir_z)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)

       CASE(c_dump_Nernst_Ex)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Nernst_electric_field(array, ispecies-avg%species_sum,c_dir_x)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Nernst_Ey)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Nernst_electric_field(array,ispecies-avg%species_sum,c_dir_y)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Nernst_Ez)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Nernst_electric_field(array, ispecies-avg%species_sum,c_dir_z)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)

       CASE(c_dump_Biermann_Ex)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Biermann_electric_field(array, ispecies-avg%species_sum,c_dir_x)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Biermann_Ey)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Biermann_electric_field(array,ispecies-avg%species_sum,c_dir_y)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_Biermann_Ez)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_Biermann_electric_field(array, ispecies-avg%species_sum,c_dir_z)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)

      CASE(c_dump_viscous_xyx_Ey)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_viscous_electric_field_xyx(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_viscous_xzx_Ez)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_viscous_electric_field_xzx(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_viscous_xyy_Ex)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_viscous_electric_field_xyy(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_viscous_yzy_Ez)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_viscous_electric_field_yzy(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_viscous_xzz_Ex)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_viscous_electric_field_xzz(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_viscous_yzz_Ey)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_viscous_electric_field_yzz(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_gyrotropy)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_gyrotropy(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) &
              + REAL(array * dt, r4)
        ENDDO
        DEALLOCATE(array)

      CASE(c_dump_rel_temperature_x)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_rel_temperature(array, ispecies-avg%species_sum,c_dir_x)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) + array * dt
        ENDDO
        DEALLOCATE(array)

      CASE(c_dump_rel_temperature_y)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_rel_temperature(array,ispecies-avg%species_sum,c_dir_y)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) + array * dt
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_rel_temperature_z)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_rel_temperature(array, ispecies-avg%species_sum,c_dir_z)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) + array * dt
        ENDDO
        DEALLOCATE(array)

      CASE(c_dump_rel_temperature_t)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_per_species_rel_temperature(array, ispecies-avg%species_sum,c_dir_t)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) + array * dt
        ENDDO
        DEALLOCATE(array)

      CASE(c_dump_magnetic_vector_potential)
        ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
        DO ispecies = 1, n_species_local
          CALL calc_magnetic_vector_potential(array, ispecies-avg%species_sum)
          avg%r4array(:,:,ispecies) = avg%r4array(:,:,ispecies) + array * dt
        ENDDO
        DEALLOCATE(array)
    ...
    ELSE
      ...
      CASE(c_dump_ez)
        ...
        !calculation
      CASE(c_dump_ey2)
        avg%array(:,:,1) = avg%array(:,:,1) + ey**2 * dt
      CASE(c_dump_ez2)
        avg%array(:,:,1) = avg%array(:,:,1) + ez**2 * dt
      CASE(c_dump_eyez)
        avg%array(:,:,1) = avg%array(:,:,1) + ey*ez * dt
      ...
      CASE(c_dump_jx0)
        avg%array(:,:,1) = avg%array(:,:,1) + jx0(1-ng:nx+ng,1-ng:ny+ng) * dt
      CASE(c_dump_jy0)
        avg%array(:,:,1) = avg%array(:,:,1) + jy0(1-ng:nx+ng,1-ng:ny+ng) * dt
      CASE(c_dump_jz0)
        avg%array(:,:,1) = avg%array(:,:,1) + jz0(1-ng:nx+ng,1-ng:ny+ng) * dt
...
!Fuild Statistics
      CASE(c_dump_rel_ux)
        ...
        DO ispecies = 1, n_species_local
          CALL calc_per_species_rel_velocity(array, ispecies-avg%species_sum,c_dir_x)
          avg%array(:,:,ispecies) = avg%array(:,:,ispecies) &
              + array * dt
        ENDDO
        DEALLOCATE(array)
      CASE(c_dump_rel_uy)
  ...
  SUBROUTINE derivation_field(ioutput, time_mid, derive)

    INTEGER, INTENT(IN) :: ioutput
    REAL(num), INTENT(IN) :: time_mid
    TYPE(derivation_data_block), INTENT(INOUT) :: derive
    INTEGER :: n_species_local, ispecies
    INTEGER :: derivation_index
    REAL(num), DIMENSION(:,:), ALLOCATABLE :: array
    

    derive%real_time = derive%real_time + dt

    derive%started = .TRUE.

    IF (derive%real_time <= time_mid) THEN
        derivation_index = -1
        derive%real_time_minus = derive%real_time_minus + dt
    ELSE
        derivation_index = 1
        derive%real_time_plus = derive%real_time_plus + dt
    END IF

    n_species_local = derive%n_species + derive%species_sum

    IF (n_species_local <= 0) RETURN

    IF (derive%dump_single) THEN
      SELECT CASE(ioutput)
      CASE(c_dump_bx)
          IF (derivation_index == -1) THEN
              derive%r4array_minus(:,:,1) = derive%r4array_minus(:,:,1) + REAL(bx * dt, r4)
          ELSE IF (derivation_index == 1) THEN
              derive%r4array_plus(:,:,1) = derive%r4array_plus(:,:,1) + REAL(bx * dt, r4)
          END IF
      CASE(c_dump_by)
          IF (derivation_index == -1) THEN
              derive%r4array_minus(:,:,1) = derive%r4array_minus(:,:,1) + REAL(by * dt, r4)
          ELSE IF (derivation_index == 1) THEN
              derive%r4array_plus(:,:,1) = derive%r4array_plus(:,:,1) + REAL(by * dt, r4)
          END IF
      CASE(c_dump_bz)
          IF (derivation_index == -1) THEN
              derive%r4array_minus(:,:,1) = derive%r4array_minus(:,:,1) + REAL(bz * dt, r4)
          ELSE IF (derivation_index == 1) THEN
              derive%r4array_plus(:,:,1) = derive%r4array_plus(:,:,1) + REAL(bz * dt, r4)
          END IF
      CASE(c_dump_ex)
          IF (derivation_index == -1) THEN
              derive%r4array_minus(:,:,1) = derive%r4array_minus(:,:,1) + REAL(ex * dt, r4)
          ELSE IF (derivation_index == 1) THEN
              derive%r4array_plus(:,:,1) = derive%r4array_plus(:,:,1) + REAL(ex * dt, r4)
          END IF
      CASE(c_dump_ey)
          IF (derivation_index == -1) THEN
              derive%r4array_minus(:,:,1) = derive%r4array_minus(:,:,1) + REAL(ey * dt, r4)
          ELSE IF (derivation_index == 1) THEN
              derive%r4array_plus(:,:,1) = derive%r4array_plus(:,:,1) + REAL(ey * dt, r4)
          END IF
      CASE(c_dump_ez)
          IF (derivation_index == -1) THEN
              derive%r4array_minus(:,:,1) = derive%r4array_minus(:,:,1) + REAL(ez * dt, r4)
          ELSE IF (derivation_index == 1) THEN
              derive%r4array_plus(:,:,1) = derive%r4array_plus(:,:,1) + REAL(ez * dt, r4)
          END IF
      CASE(c_dump_jx)
          IF (derivation_index == -1) THEN
              derive%r4array_minus(:,:,1) = derive%r4array_minus(:,:,1) + REAL(jx(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
          ELSE IF (derivation_index == 1) THEN
              derive%r4array_plus(:,:,1) = derive%r4array_plus(:,:,1) + REAL(jx(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
          END IF
      CASE(c_dump_jy)
          IF (derivation_index == -1) THEN
              derive%r4array_minus(:,:,1) = derive%r4array_minus(:,:,1) + REAL(jy(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
          ELSE IF (derivation_index == 1) THEN
              derive%r4array_plus(:,:,1) = derive%r4array_plus(:,:,1) + REAL(jy(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
          END IF
      CASE(c_dump_jz)
          IF (derivation_index == -1) THEN
              derive%r4array_minus(:,:,1) = derive%r4array_minus(:,:,1) + REAL(jz(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
          ELSE IF (derivation_index == 1) THEN
              derive%r4array_plus(:,:,1) = derive%r4array_plus(:,:,1) + REAL(jz(1-ng:nx+ng,1-ng:ny+ng) * dt, r4)
          END IF
     END SELECT
    ELSE
      SELECT CASE(ioutput)
      CASE(c_dump_bx)
          IF (derivation_index == -1) THEN
              derive%array_minus(:,:,1) = derive%array_minus(:,:,1) + bx * dt 
          ELSE IF (derivation_index == 1) THEN
              derive%array_plus(:,:,1) = derive%array_plus(:,:,1) + bx * dt
          END IF
      CASE(c_dump_by)
          IF (derivation_index == -1) THEN
              derive%array_minus(:,:,1) = derive%array_minus(:,:,1) + by * dt
          ELSE IF (derivation_index == 1) THEN
              derive%array_plus(:,:,1) = derive%array_plus(:,:,1) + by * dt
          END IF
      CASE(c_dump_bz)
          IF (derivation_index == -1) THEN
              derive%array_minus(:,:,1) = derive%array_minus(:,:,1) + bz * dt
          ELSE IF (derivation_index == 1) THEN
              derive%array_plus(:,:,1) = derive%array_plus(:,:,1) + bz * dt
          END IF
      CASE(c_dump_ex)
          IF (derivation_index == -1) THEN
              derive%array_minus(:,:,1) = derive%array_minus(:,:,1) + ex * dt
          ELSE IF (derivation_index == 1) THEN
              derive%array_plus(:,:,1) = derive%array_plus(:,:,1) + ex * dt
          END IF
      CASE(c_dump_ey)
          IF (derivation_index == -1) THEN
              derive%array_minus(:,:,1) = derive%array_minus(:,:,1) + ey * dt
          ELSE IF (derivation_index == 1) THEN
              derive%array_plus(:,:,1) = derive%array_plus(:,:,1) + ey * dt
          END IF
      CASE(c_dump_ez)
          IF (derivation_index == -1) THEN
              derive%array_minus(:,:,1) = derive%array_minus(:,:,1) + ez * dt
          ELSE IF (derivation_index == 1) THEN
              derive%array_plus(:,:,1) = derive%array_plus(:,:,1) + ez * dt
          END IF
      CASE(c_dump_jx)
          IF (derivation_index == -1) THEN
              derive%array_minus(:,:,1) = derive%array_minus(:,:,1) + jx(1-ng:nx+ng,1-ng:ny+ng) * dt
          ELSE IF (derivation_index == 1) THEN
              derive%array_plus(:,:,1) = derive%array_plus(:,:,1) + jx(1-ng:nx+ng,1-ng:ny+ng) * dt
          END IF
      CASE(c_dump_jy)
          IF (derivation_index == -1) THEN
              derive%array_minus(:,:,1) = derive%array_minus(:,:,1) + jy(1-ng:nx+ng,1-ng:ny+ng) * dt
          ELSE IF (derivation_index == 1) THEN
              derive%array_plus(:,:,1) = derive%array_plus(:,:,1) + jy(1-ng:nx+ng,1-ng:ny+ng) * dt
          END IF
      CASE(c_dump_jz)
          IF (derivation_index == -1) THEN
              derive%array_minus(:,:,1) = derive%array_minus(:,:,1) + jz(1-ng:nx+ng,1-ng:ny+ng) * dt
          ELSE IF (derivation_index == 1) THEN
              derive%array_plus(:,:,1) = derive%array_plus(:,:,1) + jz(1-ng:nx+ng,1-ng:ny+ng) * dt
          END IF
    !Four velocity
    !  CASE(c_dump_rel_uz)
    !    ALLOCATE(array(1-ng:nx+ng,1-ng:ny+ng))
    !    DO ispecies = 1, n_species_local
    !      CALL calc_per_species_rel_uz(array, ispecies-derive%species_sum)
    !      IF (derivation_index == -1) THEN
    !          derive%array_minus(:,:,ispecies) = derive%array_minus(:,:,ispecies) + array*dt  
    !      ELSE IF (derivation_index == 1) THEN
    !          derive%array_plus(:,:,ispecies) = derive%array_plus(:,:,ispecies) + array* dt
    !      END IF
    !    ENDDO
    !    DEALLOCATE(array)
   END SELECT
  ...
  END SUBROUTINE derivation_field
  ...
  SUBROUTINE write_field(id, code, block_id, name, units, stagger, array)
    ...
    IF (convert) THEN
      ...
      IF (id == c_dump_jx .OR. id == c_dump_jy .OR. id == c_dump_jz &
          .OR. id == c_dump_jx0 .OR. id == c_dump_jy0 .OR. id == c_dump_jz0) THEN
