...
CONTAINS
  ...
  SUBROUTINE auto_load
    ...
    DO ispecies = 1, n_species
      ...
      ELSE IF (species%ic_df_type == c_ic_df_power_law) THEN
        CALL setup_particle_power_law(species,species%power_law_index, species%g1,species%g2)
      ELSE IF (species%ic_df_type == c_ic_df_manual) THEN
        CALL setup_particle_manual_distribution(species,species%pc,species%g1,species%g2)
      END IF
    END DO
