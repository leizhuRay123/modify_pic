...
MODULE shared_data
  ...
  TYPE particle
    ...
    REAL(num) :: work_x = 0.0_num
    REAL(num) :: work_y = 0.0_num
    REAL(num) :: work_z = 0.0_num
    REAL(num) :: work_x_total = 0.0_num
    REAL(num) :: work_y_total = 0.0_num
    REAL(num) :: work_z_total = 0.0_num
#endif
#ifdef PHOTONS
    REAL(num) :: optical_depth
    REAL(num) , DIMENSION(3) :: stokes
    REAL(num) , DIMENSION(3) :: stokes_dir
    REAL(num) :: generate_time
    REAL(num) :: cs_times
#endif
  ...
  TYPE particle_species
    ...
    REAL(num) :: power_law_index
    REAL(num) :: g1,g2
    !For manual uniform distribution
    REAL(num) :: pc
    ...
    !Merging
    LOGICAL :: merging = .FALSE.
    INTEGER :: num_spx = 20  !default momentum grid for merging
    INTEGER :: num_spy = 20
    INTEGER :: num_spz = 10
    INTEGER :: tolerent_number = 100 !per grid | default num 
    INTEGER :: min_per_momentum = 4 !min number in momentum grid
    INTEGER :: step_merge = 500
    REAL(num) :: start_merge_time = 0.0_num
    REAL(num) :: end_merge_time = 1e30_num
    LOGICAL :: first_call_merge = .TRUE. !first time merge
  !XY
    REAL(num) :: merge_max_energy = 1.0e30_num
!xuz_merge
  ...
  TYPE derivation_data_block
    REAL(num), DIMENSION(:,:,:), POINTER :: array_plus
    REAL(r4), DIMENSION(:,:,:), POINTER :: r4array_plus
    REAL(num), DIMENSION(:,:,:), POINTER :: array_minus
    REAL(r4), DIMENSION(:,:,:), POINTER :: r4array_minus
    REAL(num) :: real_time, real_time_plus, real_time_minus
    INTEGER :: species_sum, n_species
    LOGICAL :: started, dump_single
  END TYPE derivation_data_block
  LOGICAL :: any_derivation = .FALSE.
...
!zzh
    REAL(num) :: dt_derivation, dt_min_derivation, derivation_time, derivation_time_start
    REAL(num) :: derivation_time_mid
    ...
    INTEGER :: nstep_snapshot, nstep_prev, nstep_first,nstep_average,nstep_derivation
    INTEGER :: nstep_start, nstep_stop, dump_cycle, prefix_index
    INTEGER :: dump_cycle_first_index
    LOGICAL :: restart, dump, any_average, dump_first, dump_last, any_derivation
    ...
    TYPE(derivation_data_block), DIMENSION(num_vars_to_dump) :: derivation_data
...
!zzh
  INTEGER, DIMENSION(num_vars_to_dump) :: derivation_var_block
  REAL(num) :: time_start, time_stop
  REAL(num) :: walltime_start, walltime_stop
  ...
  LOGICAL :: inject_boundaries
  REAL(num) :: inject_x_min,inject_x_max, decay_factor
...
#ifdef STATIC_FIELDS
  REAL(num), ALLOCATABLE, DIMENSION(:,:) :: ex0, ey0, ez0, bx0, by0, bz0
#endif
    !Static current
  REAL(num), ALLOCATABLE, DIMENSION(:,:) :: jx0,jy0,jz0
  REAL(num), ALLOCATABLE, DIMENSION(:,:) :: jx1,jy1,jz1
  REAL(num) :: sta_cur_time0 = 0.0_num
  REAL(num) :: sta_cur_time1 = 0.0_num
  REAL(num) :: sta_cur_timew = 0.0_num
  REAL(num) :: sta_cur_omega = 0.0_num
  REAL(num) :: sta_cur_phase = 0.0_num
  REAL(num) :: delta_jx0 = 0.0_num
  REAL(num) :: delta_jy0 = 0.0_num
  REAL(num) :: delta_jz0 = 0.0_num
  ...
  INTEGER :: n_sample_eta, n_sample_chi, n_sample_t
   !Add by XY for Polarization xi = omega/omega_c
   REAL(num), DIMENSION(3) :: dir_ob = 0.0_num ! set in control block
   REAL(num), DIMENSION(3) :: de_dir1 = 0.0_num
   REAL(num), DIMENSION(3) :: de_dir2 = 0.0_num
   REAL(num) :: dOmega = 0.0_num
   REAL(num) :: detect_energy = 0.0_num !2eV for optical range
   LOGICAL   :: nopolar_print = .TRUE.

   REAL(num) :: current_start_time = 0.0_num
   !Add by XY for optical Polarization
   REAL(num) :: optical_i1 = 0.0_num
   REAL(num) :: optical_i2 = 0.0_num
   REAL(num) :: optical_Q = 0.0_num
   REAL(num) :: optical_U = 0.0_num
   REAL(num) :: optical_band_min = 2.83974391e-19
   REAL(num) :: optical_band_max = 4.96955183e-19

  REAL(num), ALLOCATABLE :: log_xi(:),gp(:),log_y(:),log_xIntkv53(:)
  REAL(num), ALLOCATABLE :: Psi23_table(:,:), Psi13_table(:,:),kv13_div_kv23_table(:), kv23_div_Intkv53_table(:),xIntkv53_table(:)
  ! XY for syn cir polarization
  REAL(num), ALLOCATABLE ::  xkv13_table(:)
  REAL(num) :: log_xi_min,log_xi_max, log_xi_dx,gp_min,gp_max,dgp,log_y_min,log_y_max, log_dy,log_xInt_min,log_xInt_max,log_dxInt
  INTEGER :: n_sample_xi,n_sample_gp,n_sample_y,n_sample_xIntkv53,n_sample_kv23divIntkv53,n_sample_xkv13
  !XY for compton
  INTEGER :: n_sample_epo, n_sample_mu
  REAL(num), ALLOCATABLE :: Compton_table(:,:)
  REAL(num) :: log_epo_min, log_epo_max, log_depo, com_theta_min, com_theta_max,dcom_theta, com_theta
  REAL(num), ALLOCATABLE :: log_epo(:), com_mu(:,:)
  REAL(num) :: ml_energy_pho
  LOGICAL :: use_cs_times_limit = .False.
  !wein
  REAL(num), ALLOCATABLE :: wein_table(:)
  REAL(num), ALLOCATABLE :: log_wein_xi(:)
  REAL(num) :: log_wein_xi_min,log_wein_xi_max,dlog_wein_xi
  !delta_f
  REAL(num) :: delta_f_temp,delta_f_stdev,delta_f_mu
  REAL(num) :: wein_temperature
  ...
  ! XY for record photon_energy
  REAL(num) :: photon_energy_record = 0.0_num
  REAL(num) :: qed_start_time = 0.0_num
  REAL(num) :: cp_start_time = 0.0_num
  LOGICAL :: produce_pairs = .FALSE., use_radiation_reaction = .TRUE.
  LOGICAL :: produce_photons = .FALSE., photon_dynamics = .FALSE.
  LOGICAL :: use_compton_scatter = .FALSE.
  LOGICAL :: use_resonance_scatter = .FALSE.
  LOGICAL :: use_compton_recoil = .False.
  REAL(num) :: gfactor = 1.0_num  !amplified factor for qed cross section
  CHARACTER(LEN=string_length) :: qed_table_location
#endif
  LOGICAL :: use_qed = .FALSE.

!-------------------------------------------------------------------------
! PHOTON MERGE 2016.4.16 
!-----------------------------------------------------------------------
!xuz_merge
! Merging photons shared data
   LOGICAL :: merging = .FALSE.
!  INTEGER :: num_spx = 20  !default momentum grid for merging
!  INTEGER :: num_spy = 20
!  INTEGER :: num_spz = 10
!  INTEGER :: tolerent_photon_number = 100 !per grid | default num 
!  INTEGER :: min_per_momentum = 4 !min number in momentum grid
!  INTEGER :: step_merge = 500
!  INTEGER :: start_step_merge = 5000
!  LOGICAL :: first_call_merge = .TRUE. !first time merge
!  !XY
!  REAL(num) :: merge_max_energy = 1.0e30_num
!!xuz_merge
  ...
  TYPE injector_block
    ...
    REAL(num) :: power_law,g1,g2
    REAL(num) :: monoenergy,wein_temperature,angle_theta,angle_phi
    ...
    REAL(num), DIMENSION(:,:), POINTER :: depth_xy
    REAL(num), DIMENSION(:,:), POINTER :: density
    REAL(num) :: v_inject_z
  ...
  ! Optical Polarization diagnostic XY 
  REAL(num) :: total_optical_I1 = 0.0_num
  REAL(num) :: total_optical_I2 = 0.0_num
  REAL(num) :: total_optical_U = 0.0_num
  REAL(num) :: total_optical_V = 0.0_num
