...
!t Relativistic binary collision module
...
CONTAINS
  ...
  SUBROUTINE intra_collisions_sk(p_list, mass, charge, weight, &
    ...
    DO k = 1, pcount
      ...
      IF (debug_mod) THEN
      WRITE(*,*) 'rank', rank
      WRITE(*,*) 'initial p_norm',p1_norm + p2_norm
      END IF
      ...
      IF (debug_mod) THEN
      WRITE(*,*) 'After collision',wr
      WRITE(*,*) 'momentum',(p5+p6)/m0/c
    END IF
  ...
  SUBROUTINE intra_collisions_np(p_list, mass, charge, weight, &
    ...
    DO k = 1, pcount
    IF ((current%cs_times >0.0_num .OR. impact%cs_times > 0.0_num) .AND. use_cs_times_limit) CYCLE
#ifdef PER_PARTICLE_CHARGE_MASS
      m1 = current%mass
      m2 = impact%mass
      q1 = current%charge
      ...
      IF (debug_mod) THEN
      WRITE(*,*) 'np collision rank', rank
      WRITE(*,*) 'p_norm',p1_norm + p2_norm
    END IF
      ! Two stationary particles can't collide, so don't try
      IF (DOT_PRODUCT(p1_norm, p1_norm) < eps &
          .AND. DOT_PRODUCT(p2_norm, p2_norm) < eps) CYCLE
  ...
  SUBROUTINE inter_collisions_np(p_list1, p_list2, mass1, mass2, &
    ...
    IF (icount > 0 .AND. jcount > 0) THEN
      ...
      IF ((current%cs_times >0.0_num .OR. impact%cs_times > 0.0_num) .AND. use_cs_times_limit) CYCLE
        ...
        p2_norm = p2 / m0
        IF (debug_mod) THEN
          WRITE(*,*) 'np:-------',current%weight,impact%weight,wr
          WRITE(*,*) 'p1+p2:',p1_norm + p2_norm
        END IF
        ...
        ! Pre-collision energies
        !??????
        e1 = c * SQRT(DOT_PRODUCT(p1, p1)*c**2 + (m1 * c)**2)
        e2 = c * SQRT(DOT_PRODUCT(p2, p2)*c**2 + (m2 * c)**2)
  ...
  SUBROUTINE weighted_particles_correction(wtr, p, p_scat, en, en_scat, mass)
    ...
    REAL(num) :: p_after(3)
    REAL(num) :: en_after,en_after2,en1,en2;
    REAL(num) :: gamma_en, gamma_p
    REAL(num) :: delta_p, p_mag, p_trans_mag
    REAL(num) :: c1(3), c2(3), c3(3)
    REAL(num) :: phi

    !WRITE(*,*) 'In weighted correction:'
    !WRITE(*,*) 'p',p
    !WRITE(*,*) 'p_scat',p_scat
    !WRITE(*,*) 'mass',mass
    en_after = (1.0_num - wtr) * en + wtr * en_scat
    en1 = c*SQRT(DOT_PRODUCT(p,p) + (mass*c)**2)
    en2 = c*SQRT(DOT_PRODUCT(p_scat,p_scat)+(mass*c)**2)
    en_after2 = en1+en2
    ...
    ELSE
        !write(*,*) 'In:en1,en2',en,en_scat
        !write(*,*) 'calc:en1,en2',en1,en2
        !write(*,*) 'gamma_p > gamma_en',gamma_p,gamma_en
        !write(*,*) 'pscat',p_scat
        !write(*,*) 'p_after',p_after
        p_scat = p_after
    END IF
