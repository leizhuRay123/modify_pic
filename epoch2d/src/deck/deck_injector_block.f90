...
CONTAINS
  ...
  FUNCTION injector_block_handle_element(element, value) RESULT(errcode)
    ...
    REAL(num) :: mult
    REAL(num) :: dummy
    CHARACTER(LEN=string_length) :: mult_string,filename
    ...
    IF (str_cmp(element, 'v_inject_z'))  THEN
      working_injector%v_inject_z = as_real_print(value, element, errcode)
      RETURN
    END IF

    !Copy t_profile in laser_deck
    IF (str_cmp(element, 't_profile'))  THEN
      working_injector%use_time_function  = .True.
      CALL initialise_stack(working_injector%time_function)
      CALL tokenize(value, working_injector%time_function, errcode)
      ! evaluate it once to check that it's a valid block
      dummy = evaluate(working_injector%time_function, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'power_law'))  THEN
      working_injector%power_law = as_real_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'g1'))  THEN
      working_injector%g1 = as_real_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'g2'))  THEN
      working_injector%g2 = as_real_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'wein_temperature'))  THEN
      working_injector%wein_temperature = as_real_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'monoenergy'))  THEN
      working_injector%monoenergy = as_real_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'angle_theta'))  THEN
      working_injector%angle_theta = as_real_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'angle_phi'))  THEN
      working_injector%angle_phi = as_real_print(value, element, errcode)
      RETURN
    END IF
