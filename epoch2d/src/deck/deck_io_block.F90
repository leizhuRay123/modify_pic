...
CONTAINS
  ...
  SUBROUTINE io_block_end
    ...
    io_block%dumpmask(c_dump_jx0) = IOR(io_block%dumpmask(c_dump_jx0), c_io_field)
    io_block%dumpmask(c_dump_jy0) = IOR(io_block%dumpmask(c_dump_jy0), c_io_field)
    io_block%dumpmask(c_dump_jz0) = IOR(io_block%dumpmask(c_dump_jz0), c_io_field)
  ...
  FUNCTION io_block_handle_element(element, value) RESULT(errcode)
    ...
    ! For stokes and generate_time
    ELSE IF (str_cmp(element, 'generate_time')) THEN
      elementselected = c_dump_part_generate_time

    ELSE IF (str_cmp(element, 'cs_times')) THEN
      elementselected = c_dump_part_cs_times

    ELSE IF (str_cmp(element, 'stokes_I1')) THEN
      elementselected = c_dump_part_stokes_I1

    ELSE IF (str_cmp(element, 'stokes_I2')) THEN
      elementselected = c_dump_part_stokes_I2

    ELSE IF (str_cmp(element, 'stokes_V')) THEN
      elementselected = c_dump_part_stokes_V
      
    ELSE IF (str_cmp(element, 'stokes_dirx')) THEN
      elementselected = c_dump_part_stokes_dirx
    ELSE IF (str_cmp(element, 'stokes_diry')) THEN
      elementselected = c_dump_part_stokes_diry
    ELSE IF (str_cmp(element, 'stokes_dirz')) THEN
      elementselected = c_dump_part_stokes_dirz
#endif
    ...
    ELSE IF (str_cmp(element, 'ey2')) THEN
      elementselected = c_dump_ey2

    ELSE IF (str_cmp(element, 'ez2')) THEN
      elementselected = c_dump_ez2

    ELSE IF (str_cmp(element, 'eyez')) THEN
      elementselected = c_dump_eyez
    ...
    ELSE IF (str_cmp(element, 'jx0')) THEN
      elementselected = c_dump_jx0

    ELSE IF (str_cmp(element, 'jy0')) THEN
      elementselected = c_dump_jy0

    ELSE IF (str_cmp(element, 'jz0')) THEN
      elementselected = c_dump_jz0
    ...
    ELSE IF (str_cmp(element, 'polar_degree')) THEN
      elementselected = c_dump_polar_degree

    ELSE IF (str_cmp(element, 'polar_angle')) THEN
      elementselected = c_dump_polar_angle

    ELSE IF (str_cmp(element, 'stokes_I')) THEN
      elementselected = c_dump_stokes_I

    ELSE IF (str_cmp(element, 'stokes_Q')) THEN
      elementselected = c_dump_stokes_Q

    ELSE IF (str_cmp(element, 'stokes_U')) THEN
      elementselected = c_dump_stokes_U

    ELSE IF (str_cmp(element, 'stokes_N')) THEN
      elementselected = c_dump_stokes_N

    ! for optical polarization
    ELSE IF (str_cmp(element, 'optical_stokes_I')) THEN
      elementselected = c_dump_optical_stokes_I
    ELSE IF (str_cmp(element, 'optical_stokes_Q')) THEN
      elementselected = c_dump_optical_stokes_Q
    ELSE IF (str_cmp(element, 'optical_stokes_U')) THEN
      elementselected = c_dump_optical_stokes_U
    ELSE IF (str_cmp(element, 'optical_stokes_V')) THEN
      elementselected = c_dump_optical_stokes_V
    ELSE IF (str_cmp(element, 'optical_stokes_N')) THEN
      elementselected = c_dump_optical_stokes_N
      !for i1detect
    ELSE IF (str_cmp(element, 'i1_detect')) THEN
      elementselected = c_dump_i1_detect

    ELSE IF (str_cmp(element, 'i2_detect')) THEN
      elementselected = c_dump_i2_detect
    ...
    ELSE IF (str_cmp(element, 'total_optical_intensity_sum')) THEN
      elementselected = c_dump_total_optical_intensity_sum

    ELSE IF (str_cmp(element, 'rel_temperature_x')) THEN
      elementselected = c_dump_rel_temperature_x

    ELSE IF (str_cmp(element, 'rel_temperature_y')) THEN
      elementselected = c_dump_rel_temperature_y

    ELSE IF (str_cmp(element, 'rel_temperature_z')) THEN
      elementselected = c_dump_rel_temperature_z

    ELSE IF (str_cmp(element, 'rel_temperature_t')) THEN
      elementselected = c_dump_rel_temperature_t

    ELSE IF (str_cmp(element, 'velocity_ux')) THEN
      elementselected = c_dump_ux
    ELSE IF (str_cmp(element, 'velocity_uy')) THEN
      elementselected = c_dump_uy
    ELSE IF (str_cmp(element, 'velocity_uz')) THEN
      elementselected = c_dump_uz

    ELSE IF (str_cmp(element, 'four_velocity_ux')) THEN
      elementselected = c_dump_rel_ux
    ELSE IF (str_cmp(element, 'four_velocity_uy')) THEN
      elementselected = c_dump_rel_uy
    ELSE IF (str_cmp(element, 'four_velocity_uz')) THEN
      elementselected = c_dump_rel_uz

    ELSE IF (str_cmp(element, 'pressure_pxx')) THEN
      elementselected = c_dump_Pxx
    ELSE IF (str_cmp(element, 'pressure_pxy')) THEN
      elementselected = c_dump_Pxy
    ELSE IF (str_cmp(element, 'pressure_pxz')) THEN
      elementselected = c_dump_Pxz
    ELSE IF (str_cmp(element, 'pressure_pyy')) THEN
      elementselected = c_dump_Pyy
    ELSE IF (str_cmp(element, 'pressure_pyz')) THEN
      elementselected = c_dump_Pyz
    ELSE IF (str_cmp(element, 'pressure_pzz')) THEN
      elementselected = c_dump_Pzz

    ELSE IF (str_cmp(element, 'thermal_flux')) THEN
      elementselected = c_dump_thermalflux

    ELSE IF (str_cmp(element, 'gyrotropy')) THEN
      elementselected = c_dump_gyrotropy

    ELSE IF (str_cmp(element, 'Hall_ex')) THEN
      elementselected = c_dump_Hall_Ex
    ELSE IF (str_cmp(element, 'Hall_ey')) THEN
      elementselected = c_dump_Hall_Ey
    ELSE IF (str_cmp(element, 'Hall_ez')) THEN
      elementselected = c_dump_Hall_Ez

    ELSE IF (str_cmp(element, 'Nernst_ex')) THEN
      elementselected = c_dump_Nernst_Ex
    ELSE IF (str_cmp(element, 'Nernst_ey')) THEN
      elementselected = c_dump_Nernst_Ey
    ELSE IF (str_cmp(element, 'Nernst_ez')) THEN
      elementselected = c_dump_Nernst_Ez

    ELSE IF (str_cmp(element, 'Biermann_ex')) THEN
      elementselected = c_dump_Biermann_Ex
    ELSE IF (str_cmp(element, 'Biermann_ey')) THEN
      elementselected = c_dump_Biermann_Ey
    ELSE IF (str_cmp(element, 'Biermann_ez')) THEN
      elementselected = c_dump_Biermann_Ez

    ELSE IF (str_cmp(element, 'viscous_xyx_ey')) THEN
      elementselected = c_dump_viscous_xyx_Ey
    ELSE IF (str_cmp(element, 'viscous_xzx_ez')) THEN
      elementselected = c_dump_viscous_xzx_Ez
    ELSE IF (str_cmp(element, 'viscous_xyy_ex')) THEN
      elementselected = c_dump_viscous_xyy_Ex
    ELSE IF (str_cmp(element, 'viscous_yzy_ez')) THEN
      elementselected = c_dump_viscous_yzy_Ez
    ELSE IF (str_cmp(element, 'viscous_xzz_ex')) THEN
      elementselected = c_dump_viscous_xzz_Ex
    ELSE IF (str_cmp(element, 'viscous_yzz_ey')) THEN
      elementselected = c_dump_viscous_yzz_Ey

    ELSE IF (str_cmp(element, 'convection_ex')) THEN
      elementselected = c_dump_convection_Ex
    ELSE IF (str_cmp(element, 'convection_ey')) THEN
      elementselected = c_dump_convection_Ey
    ELSE IF (str_cmp(element, 'convection_ez')) THEN
      elementselected = c_dump_convection_Ez
    ELSE IF (str_cmp(element, 'magnetic_vector_potential')) THEN
      elementselected = c_dump_magnetic_vector_potential
    ELSE
      got_element = .FALSE.
    END IF
...
#define PARTICLE_ID
      ...
      IF (IAND(mask, c_io_species) /= 0 &
        ...
        IF (mask_element == c_dump_i1_detect) bad = .FALSE.
        IF (mask_element == c_dump_i2_detect) bad = .FALSE.
        IF (mask_element == c_dump_polar_degree) bad = .FALSE.
        IF (mask_element == c_dump_polar_angle) bad = .FALSE.
        IF (mask_element == c_dump_stokes_I) bad = .FALSE.
        IF (mask_element == c_dump_stokes_U) bad = .FALSE.
        IF (mask_element == c_dump_stokes_Q) bad = .FALSE.
        IF (mask_element == c_dump_stokes_N) bad = .FALSE.
        IF (mask_element == c_dump_optical_stokes_I) bad = .FALSE.
        IF (mask_element == c_dump_optical_stokes_U) bad = .FALSE.
        IF (mask_element == c_dump_optical_stokes_Q) bad = .FALSE.
        IF (mask_element == c_dump_optical_stokes_V) bad = .FALSE.
        IF (mask_element == c_dump_optical_stokes_N) bad = .FALSE.
        ...
        IF (mask_element == c_dump_jx0) bad = .FALSE.
        IF (mask_element == c_dump_jy0) bad = .FALSE.
        IF (mask_element == c_dump_jz0) bad = .FALSE.
        IF (mask_element == c_dump_total_energy_sum) bad = .FALSE.
        IF (mask_element == c_dump_total_optical_intensity_sum) bad = .FALSE.

        IF (mask_element == c_dump_ux) bad = .FALSE.
        IF (mask_element == c_dump_uy) bad = .FALSE.
        IF (mask_element == c_dump_uz) bad = .FALSE.
        IF (mask_element == c_dump_rel_ux) bad = .FALSE.
        IF (mask_element == c_dump_rel_uy) bad = .FALSE.
        IF (mask_element == c_dump_rel_uz) bad = .FALSE.
        IF (mask_element == c_dump_Pxx) bad = .FALSE.
        IF (mask_element == c_dump_Pxy) bad = .FALSE.
        IF (mask_element == c_dump_Pxz) bad = .FALSE.
        IF (mask_element == c_dump_Pyy) bad = .FALSE.
        IF (mask_element == c_dump_Pyz) bad = .FALSE.
        IF (mask_element == c_dump_Pzz) bad = .FALSE.
        IF (mask_element == c_dump_thermalflux) bad = .FALSE.
        IF (mask_element == c_dump_gyrotropy) bad = .FALSE.
        IF (mask_element == c_dump_Hall_Ex) bad = .FALSE.
        IF (mask_element == c_dump_Hall_Ey) bad = .FALSE.
        IF (mask_element == c_dump_Hall_Ez) bad = .FALSE.
        IF (mask_element == c_dump_Nernst_Ex) bad = .FALSE.
        IF (mask_element == c_dump_Nernst_Ey) bad = .FALSE.
        IF (mask_element == c_dump_Nernst_Ez) bad = .FALSE.
        IF (mask_element == c_dump_Biermann_Ex) bad = .FALSE.
        IF (mask_element == c_dump_Biermann_Ey) bad = .FALSE.
        IF (mask_element == c_dump_Biermann_Ez) bad = .FALSE.
        IF (mask_element == c_dump_viscous_xyx_Ey) bad = .FALSE.
        IF (mask_element == c_dump_viscous_xzx_Ez) bad = .FALSE.
        IF (mask_element == c_dump_viscous_xyy_Ex) bad = .FALSE.
        IF (mask_element == c_dump_viscous_yzy_Ez) bad = .FALSE.
        IF (mask_element == c_dump_viscous_xzz_Ex) bad = .FALSE.
        IF (mask_element == c_dump_viscous_yzz_Ey) bad = .FALSE.
        IF (mask_element == c_dump_convection_Ex) bad = .FALSE.
        IF (mask_element == c_dump_convection_Ey) bad = .FALSE.
        IF (mask_element == c_dump_convection_Ez) bad = .FALSE.
        IF (mask_element == c_dump_rel_temperature_x) bad = .FALSE.
        IF (mask_element == c_dump_rel_temperature_y) bad = .FALSE.
        IF (mask_element == c_dump_rel_temperature_z) bad = .FALSE.
        IF (mask_element == c_dump_rel_temperature_t) bad = .FALSE.
        IF (mask_element == c_dump_magnetic_vector_potential) bad = .FALSE.
      ...
      IF (IAND(mask, c_io_averaged) /= 0) THEN
        ...
        !! must add for laser stokes
        IF (mask_element == c_dump_ey2) bad = .FALSE.
        IF (mask_element == c_dump_ez2) bad = .FALSE.
        IF (mask_element == c_dump_eyez) bad = .FALSE.
  ...
  SUBROUTINE set_restart_dumpmasks
    ...
    io_block%dumpmask(c_dump_jx0) = &
        IOR(io_block%dumpmask(c_dump_jx0), c_io_restartable)
    io_block%dumpmask(c_dump_jy0) = &
        IOR(io_block%dumpmask(c_dump_jy0), c_io_restartable)
    io_block%dumpmask(c_dump_jz0) = &
        IOR(io_block%dumpmask(c_dump_jz0), c_io_restartable)
    ! CPML boundaries
