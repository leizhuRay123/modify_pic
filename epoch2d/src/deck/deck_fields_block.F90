...
CONTAINS
  ...
  FUNCTION fields_block_handle_element(element, value) RESULT(errcode)
...
#ifdef STATIC_FIELDS
        CALL load_single_array_from_file(filename, ex0, offset, errcode)
#endif
...
#ifdef STATIC_FIELDS
        CALL load_single_array_from_file(filename, ey0, offset, errcode)
#endif
      ELSE
...
#ifdef STATIC_FIELDS
        CALL load_single_array_from_file(filename, ez0, offset, errcode)
#endif
      ELSE
...
#ifdef STATIC_FIELDS
        CALL load_single_array_from_file(filename, bx0, offset, errcode)
#endif
      ELSE
...
#ifdef STATIC_FIELDS
        CALL load_single_array_from_file(filename, by0, offset, errcode)
#endif
      ELSE
...
#ifdef STATIC_FIELDS
        CALL load_single_array_from_file(filename, bz0, offset, errcode)
#endif
      ELSE
    ...
    IF (str_cmp(element, 'jx')) THEN
      IF (got_file) THEN
        CALL load_single_array_from_file(filename, jx0, offset, errcode)
      ELSE
        CALL set_tokenizer_stagger(c_stagger_ex)
        CALL evaluate_string_in_space(value, jx0, 1-ng, nx+ng, 1-ng, ny+ng, &
            errcode)
        CALL set_tokenizer_stagger(c_stagger_centre)
      END IF
      RETURN
    ...
    IF (str_cmp(element, 'jy')) THEN
      IF (got_file) THEN
        CALL load_single_array_from_file(filename, jy0, offset, errcode)
      ELSE
        CALL set_tokenizer_stagger(c_stagger_ex)
        CALL evaluate_string_in_space(value, jy0, 1-ng, nx+ng, 1-ng, ny+ng, &
            errcode)
        CALL set_tokenizer_stagger(c_stagger_centre)
      END IF
      RETURN
    ...
    IF (str_cmp(element, 'jz')) THEN
      IF (got_file) THEN
        CALL load_single_array_from_file(filename, jz0, offset, errcode)
      ELSE
        CALL set_tokenizer_stagger(c_stagger_ex)
        CALL evaluate_string_in_space(value, jz0, 1-ng, nx+ng, 1-ng, ny+ng, &
            errcode)
        CALL set_tokenizer_stagger(c_stagger_centre)
      END IF
      RETURN
    ...
    IF (str_cmp(element, 'jx1')) THEN
      IF (got_file) THEN
        CALL load_single_array_from_file(filename, jx1, offset, errcode)
      ELSE
        CALL set_tokenizer_stagger(c_stagger_ex)
        CALL evaluate_string_in_space(value, jx1, 1-ng, nx+ng, 1-ng, ny+ng, &
            errcode)
        CALL set_tokenizer_stagger(c_stagger_centre)
      END IF
      RETURN
    ...
    IF (str_cmp(element, 'jy1')) THEN
      IF (got_file) THEN
        CALL load_single_array_from_file(filename, jy1, offset, errcode)
      ELSE
        CALL set_tokenizer_stagger(c_stagger_ex)
        CALL evaluate_string_in_space(value, jy1, 1-ng, nx+ng, 1-ng, ny+ng, &
            errcode)
        CALL set_tokenizer_stagger(c_stagger_centre)
      END IF
      RETURN
    ...
    IF (str_cmp(element, 'jz1')) THEN
      IF (got_file) THEN
        CALL load_single_array_from_file(filename, jz1, offset, errcode)
      ELSE
        CALL set_tokenizer_stagger(c_stagger_ex)
        CALL evaluate_string_in_space(value, jz1, 1-ng, nx+ng, 1-ng, ny+ng, &
            errcode)
        CALL set_tokenizer_stagger(c_stagger_centre)
      END IF
      RETURN
