...
CONTAINS
  ...
  SUBROUTINE control_deck_initialise
    ...
    !de_dir2 = dir_ob \times de_dir1
    de_dir2 = (/ 0.0_num,1.0_num,0.0_num /) 
    dOmega = 4*pi/720.0
    detect_energy = 2*q0 !2*q0 for optical energy
    !for neutral jet injection
    current_start_time = 0.0_num
    plasma_state = .TRUE.
    !wein_temp
    wein_temperature = -1.0_num
    !delta_f for photon
    delta_f_temp = -1.0_num
    delta_f_stdev = -1.0_num
    delta_f_mu = -1.0_num
    !for sta_current
    sta_cur_time0 = 0.0_num
    sta_cur_time1 = 0.0_num
    sta_cur_timew = 1000000000.0_num
    sta_cur_omega = 0.0_num
    sta_cur_phase = 0.0_num
    delta_jx0 = 0.0_num
    delta_jy0 = 0.0_num
    delta_jz0 = 0.0_num
    debug_mod = .FALSE.
  ...
  SUBROUTINE control_block_end
    ...
    !de_dir2 = dir_ob \times de_dir1
    de_dir2 = (/ dir_ob(2)*de_dir1(3) - dir_ob(3)*de_dir1(2), &
                 dir_ob(3)*de_dir1(1) - dir_ob(1)*de_dir1(3), &
                 dir_ob(1)*de_dir1(2) - dir_ob(2)*de_dir1(1) /)
    IF( rank == 0 .AND. nopolar_print) THEN
        WRITE(*,*) '********************Polarization module********************'
        WRITE(*,100) dir_ob(1),dir_ob(2),dir_ob(3)
        WRITE(*,200) de_dir1(1),de_dir1(2),de_dir1(3)
        WRITE(*,300) de_dir2(1),de_dir2(2),de_dir2(3)
        WRITE(*,400) detect_energy/q0
        WRITE(*,500) 4*pi/720
        WRITE(*,*) '********************Polarization module********************'
        100 FORMAT('  ob_dir   = (',3F5.2,')')
        200 FORMAT('  dir1(I1) = (',3F5.2,')')
        300 FORMAT('  dir2(I2) = (',3F5.2,')')
        400 FORMAT('  detect_energy = (',F5.2,')')
        500 FORMAT('  dOmega   = (',F5.2,')')
        nopolar_print = .False.
    END IF
  ...
  FUNCTION control_block_handle_element(element, value) RESULT(errcode)
    ...
    ELSE IF (str_cmp(element, 'de_dir1x')) THEN
      de_dir1(1) = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'de_dir1y')) THEN
      de_dir1(2) = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'de_dir1z')) THEN
      de_dir1(3) = as_real_print(value, element, errcode)

    ELSE IF (str_cmp(element, 'dir_obx')) THEN
      dir_ob(1) = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'dir_oby')) THEN
      dir_ob(2) = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'dir_obz')) THEN
      dir_ob(3) = as_real_print(value, element, errcode)

    ELSE IF (str_cmp(element, 'dOmega')) THEN
        dOmega = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'detect_energy')) THEN
        detect_energy = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'current_start_time')) THEN
        current_start_time = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'plasma_state')) THEN
      plasma_state = as_logical_print(value, element, errcode)
        ! for wein distribution of photon
    ELSE IF (str_cmp(element, 'wein_temperature')) THEN
      wein_temperature = as_real_print(value, element, errcode)
        ! for delta distribution of photon
    ELSE IF (str_cmp(element, 'delta_f_temp')) THEN
      delta_f_temp = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'delta_f_stdev')) THEN
      delta_f_stdev = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'delta_f_mu')) THEN
      delta_f_mu = as_real_print(value, element, errcode)

      !For sta_current
    ELSE IF (str_cmp(element, 'sta_cur_time0')) THEN
      sta_cur_time0 = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'sta_cur_time1')) THEN
      sta_cur_time1 = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'sta_cur_timew')) THEN
      sta_cur_timew = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'sta_cur_omega')) THEN
      sta_cur_omega = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'sta_cur_phase')) THEN
      sta_cur_phase = as_real_print(value, element, errcode)

    ELSE IF (str_cmp(element, 'delta_jx0')) THEN
      delta_jx0 = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'delta_jy0')) THEN
      delta_jy0 = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'delta_jz0')) THEN
      delta_jz0 = as_real_print(value, element, errcode)
    ELSE IF (str_cmp(element, 'debug_mod')) THEN
      debug_mod = as_logical_print(value, element, errcode)
