...
CONTAINS
  ...
  SUBROUTINE qed_deck_initialise
    ...
    IF (deck_state == c_ds_first) THEN
      ...
      cp_start_time = 0.0_num
      produce_pairs = .FALSE.
      use_radiation_reaction = .TRUE.
      produce_photons = .FALSE.
      photon_dynamics = .FALSE.
      use_compton_scatter = .FALSE.
      use_resonance_scatter = .FALSE.
      use_compton_recoil = .False.
      use_cs_times_limit = .False.
      gfactor = 1.0
      ml_energy_pho = 1.0_num*m0*c**2
    END IF
#endif
  ...
  FUNCTION qed_block_handle_element(element, value) RESULT(errcode)
    ...
    IF (str_cmp(element, 'cp_start_time')) THEN
      cp_start_time = as_real_print(value, element, errcode)
      RETURN
    END IF
    ...
    IF (str_cmp(element, 'use_compton_scatter')) THEN
      use_compton_scatter = as_logical_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'use_resonance_scatter')) THEN
      use_resonance_scatter = as_logical_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'use_compton_recoil')) THEN
      use_compton_recoil = as_logical_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'ml_energy_pho')) THEN
      ml_energy_pho = as_real_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'gfactor')) THEN
      gfactor = as_real_print(value, element, errcode)
      RETURN
    END IF
    !debug use
    IF (str_cmp(element, 'use_cs_times_limit')) THEN
      use_cs_times_limit = as_logical_print(value, element, errcode)
      RETURN
    END IF

    IF (str_cmp(element, 'merging')) THEN
      merging = as_logical_print(value, element, errcode)
      RETURN
    END IF
