    ...
    DO WHILE (ASSOCIATED(current_list) .AND. (part_count < npoint_it))
      ...
      CASE (c_dump_part_ek)
        ...
        ELSE
          DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
            part_count = part_count + 1
            array(part_count) = c*SQRT(SUM(cur%part_p**2))
            cur => cur%next
          END DO
#endif
        END IF
      ...
      CASE (c_dump_part_stokes_I1)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%stokes(1)
          cur => cur%next
        END DO
        
      CASE (c_dump_part_stokes_I2)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%stokes(2)
          cur => cur%next
        END DO

      CASE (c_dump_part_stokes_V)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%stokes(3)
          cur => cur%next
        END DO

      CASE (c_dump_part_stokes_dirx)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%stokes_dir(1)
          cur => cur%next
        END DO

      CASE (c_dump_part_stokes_diry)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%stokes_dir(2)
          cur => cur%next
        END DO

      CASE (c_dump_part_stokes_dirz)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%stokes_dir(3)
          cur => cur%next
        END DO

      CASE (c_dump_part_generate_time)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%generate_time
          cur => cur%next
        END DO

      CASE (c_dump_part_cs_times)
        DO WHILE (ASSOCIATED(cur) .AND. (part_count < npoint_it))
          part_count = part_count + 1
          array(part_count) = cur%cs_times
          cur => cur%next
        END DO
#endif
