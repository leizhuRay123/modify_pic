...
#include "particle_to_grid.inc"
    ...
    !WRITE(*,*) 'temperature:', part_species%species_type, partlist%count, ipart
...
#include "particle_to_grid.inc"
  ...
  ! Subroutine to initialise an power law distribution function
  SUBROUTINE setup_particle_power_law(part_species,n, g1,g2)

    TYPE(particle_species), POINTER :: part_species
    REAL(num), INTENT(IN) :: g1, g2
    TYPE(particle_list), POINTER :: partlist
    TYPE(particle), POINTER :: current
    REAL(num), INTENT(IN) :: n
    REAL(num) :: gam, P,theta,phi,px,py,pz,mass,en
    INTEGER :: ispecies
    INTEGER(i8) :: ipart, it, ipart_global, it_global
    REAL(num) :: average_its
    INTEGER :: ix, iy, idir, err
...
!
    ...
    IF (rank == 0 .AND. average_its >= max_average_its) THEN
      ...
      END DO
    END IF
  END SUBROUTINE setup_particle_power_law

  SUBROUTINE  setup_particle_manual_distribution(part_species,pc,g1,g2)
    TYPE(particle_species), POINTER :: part_species
    TYPE(particle_list), POINTER :: partlist
    TYPE(particle), POINTER :: current
    REAL(num), INTENT(IN) :: pc,g1,g2
    REAL(num) :: gam, P,theta,phi,px,py,pz,mass,en,cdf,pdf
    INTEGER :: ispecies
    INTEGER(i8) :: ipart, it, ipart_global, it_global
    REAL(num) :: average_its
    INTEGER :: ix, iy, idir, err
...
#include "particle_to_grid.inc"
      ...
      parameters%pack_pos = current%part_pos
      err = 0
            cdf = exp(-g1/pc) - exp(-g2/pc)
            P = random()
            pdf = exp(-g1/pc) - P*CDF
            px = 0.0_num
            py = -LOG(pdf)*pc/mass/c
            gam = sqrt((px)**2 + 1.0_num)
            pz = 0.0_num
    ...
    IF (rank == 0 .AND. average_its >= max_average_its) THEN
      ...
      END DO
    END IF
  END SUBROUTINE setup_particle_manual_distribution
