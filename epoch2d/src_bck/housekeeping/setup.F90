...
CONTAINS
  ...
  SUBROUTINE minimal_init
    ...
    !time_Profile XY for sta_cur
    sta_cur_time0 = 0.0_num
    sta_cur_time1 = 0.0_num
    sta_cur_timew = 0.0_num
  ...
  SUBROUTINE setup_data_derivating()

    INTEGER :: io, ib, nspec_local, nstep_derivation, mask
    REAL(num) :: dt_derivation
    TYPE(derivation_data_block), POINTER :: derive

    IF (.NOT. any_derivation) RETURN

    DO io = 1, n_io_blocks
      IF (io_block_list(io)%any_derivation) THEN
        dt_min_derivation = t_end
      ELSE
        dt_min_derivation = -1.0_num
      ENDIF
    ENDDO

    DO io = 1, num_vars_to_dump
      ib = derivation_var_block(io)
      IF (ib /= 0) THEN
        dt_derivation  = io_block_list(ib)%dt_derivation
        nstep_derivation = io_block_list(ib)%nstep_derivation

        IF (nstep_derivation > 0 .AND. dt_derivation > 0) THEN
          io_block_list(ib)%dt_min_derivation = &
              MIN(io_block_list(ib)%dt_min_derivation, &
              dt_derivation / REAL(nstep_derivation, num))
        ENDIF
        ...
        derive => io_block_list(ib)%derivation_data(io)
        IF (derive%dump_single) THEN
          ALLOCATE(derive%r4array_plus(1-ng:nx+ng, 1-ng:ny+ng, nspec_local))
          ALLOCATE(derive%r4array_minus(1-ng:nx+ng, 1-ng:ny+ng, nspec_local))
          derive%r4array_plus = 0.0_num
          derive%r4array_minus = 0.0_num
        ELSE
          ALLOCATE(derive%array_plus(1-ng:nx+ng, 1-ng:ny+ng, nspec_local))
          ALLOCATE(derive%array_minus(1-ng:nx+ng, 1-ng:ny+ng, nspec_local))
          derive%array_plus = 0.0_num
          derive%array_minus = 0.0_num
        ENDIF

        derive%species_sum = 0
        derive%n_species = 0
        IF (IAND(mask, c_io_no_sum) == 0) &
            derive%species_sum = 1
        IF (IAND(mask, c_io_species) /= 0) &
            derive%n_species = n_species
        derive%real_time = 0.0_num
        derive%started = .FALSE.
      ENDIF
    ENDDO

  END SUBROUTINE setup_data_derivating
  ...
  SUBROUTINE setup_species
    ...
    DO ispecies = 1, n_species
      ...
      species_list(ispecies)%inject_in_z = .FALSE.
      species_list(ispecies)%immobile_time = 0.0_num
  ...
  SUBROUTINE set_plasma_frequency_dt
    ...
    REAL(num) :: min_dt, omega2, omega, k_max, fac1, fac2, clipped_dens,vte2
    ...
    DO ispecies = 1, n_species
      IF (species_list(ispecies)%species_type /= c_species_id_photon) THEN
        ...
        !XY 
        fac2 = 3.0_num * k_max**2 
        IF (ic%density_max > 0) THEN
          DO iy = 1, ny
          DO ix = 1, nx
            vte2 = kb*MAXVAL(species_temp(ix,iy,:))/ species_list(ispecies)%mass
            IF(vte2 > c) vte2 = c 
  ...
  SUBROUTINE set_dt        ! sets CFL limited step
    ...
    END IF
    IF (rank == 0) THEN
        WRITE(*,*) 'dt_plasma_frequency', dt_plasma_frequency
        WRITE(*,*) 'dt_laser', dt_laser
        WRITE(*,*) 'cfl dt', dt
    END IF
    ...
    IF (dt_plasma_frequency > c_tiny .AND. plasma_state) dt = MIN(dt, dt_plasma_frequency)
