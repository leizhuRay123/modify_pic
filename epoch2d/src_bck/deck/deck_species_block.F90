...
CONTAINS
  ...
  FUNCTION species_block_handle_element(element, value) RESULT(errcode)
    ...
    IF (str_cmp(element, 'fractional_tail_cutoff')) THEN
      ...
      RETURN
    END IF
! XY for power_law__index
    IF (str_cmp(element, 'power_law_index')) THEN
      species_list(species_id)%ic_df_type  = c_ic_df_power_law
      species_list(species_id)%power_law_index  = as_real_print(value, element, errcode)
      species_list(species_id)%dist_fn%should_simplify = .FALSE.
      RETURN
    END IF !pli


    IF (str_cmp(element, 'g1')) THEN
      species_list(species_id)%g1  = as_real_print(value, element, errcode)
      RETURN
    END IF !g2

    IF (str_cmp(element, 'g2')) THEN
      species_list(species_id)%g2  = as_real_print(value, element, errcode)
      RETURN
    END IF !g2

    IF (str_cmp(element, 'pc')) THEN
      species_list(species_id)%ic_df_type  = c_ic_df_manual
      species_list(species_id)%pc  = as_real_print(value, element, errcode)
      species_list(species_id)%dist_fn%should_simplify = .FALSE.
      RETURN
    END IF !pc uniform 
    ...
    !XY for immobile_time
    IF (str_cmp(element, 'immobile_time')) THEN
      species_list(species_id)%immobile_time = &
          as_real_print(value, element, errcode)
      RETURN
    END IF
    ...
    IF (str_cmp(element, 'inject_in_z')) THEN
      species_list(species_id)%inject_in_z = &
          as_logical_print(value, element, errcode)
      !WRITE(*,*) 'species', species_id, species_list(species_id)%inject_in_z
      RETURN
    END IF
    ...
    END IF
    ! *************************************************************
    ! This section sets properties for particle merge
    ! *************************************************************
    !-------Merging by Xu Zheng
    !xuz_merge
        IF (str_cmp(element, 'merging')) THEN
      species_list(species_id)%merging = as_logical_print(value, element, errcode)
      RETURN
    ENDIF

    IF (str_cmp(element, 'num_spx'))  THEN
      species_list(species_id)%num_spx = as_integer_print(value, element, errcode)
      RETURN
    ENDIF

    IF (str_cmp(element, 'num_spy'))  THEN
      species_list(species_id)%num_spy = as_integer_print(value, element, errcode)
      RETURN
    ENDIF

        IF (str_cmp(element, 'num_spz'))  THEN
      species_list(species_id)%num_spz = as_integer_print(value, element, errcode)
      RETURN
    ENDIF

    IF (str_cmp(element, 'tolerent_number'))  THEN
      species_list(species_id)%tolerent_number = as_integer_print(value, element, errcode)
      RETURN
    ENDIF

    IF (str_cmp(element, 'min_per_momentum'))  THEN
      species_list(species_id)%min_per_momentum = as_integer_print(value, element, errcode)
      RETURN
    ENDIF

    IF (str_cmp(element, 'step_merge'))  THEN
      species_list(species_id)%step_merge = as_integer_print(value, element, errcode)
      RETURN
    ENDIF

    IF (str_cmp(element, 'start_merge_time'))  THEN
      species_list(species_id)%start_merge_time = as_real_print(value, element, errcode)
      RETURN
    ENDIF

    IF (str_cmp(element, 'end_merge_time'))  THEN
      species_list(species_id)%end_merge_time = as_real_print(value, element, errcode)
      RETURN
    ENDIF
    !XY add pmax
    IF (str_cmp(element, 'merge_max_energy'))  THEN
      species_list(species_id)%merge_max_energy = as_real_print(value, element, errcode)
      RETURN
    ENDIF
