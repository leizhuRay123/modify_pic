...
CONTAINS
  ...
  SUBROUTINE push_particles
    ...
    DO ispecies = 1, n_species
      ...
      IF (time < species_list(ispecies)%immobile_time) CYCLE
      ...
      DO ipart = 1, species_list(ispecies)%attached_list%count
        ...
        !WRITE(*,*) 'current_part_x,part_y:',current%part_pos
        !WRITE(*,*) 'current_part_p:',current%part_p
        !WRITE(*,*) 'current_id:',current%id
...
#include "triangle/hx_dcell.inc"
          ...
          !XY 假设密度是实验室系下观测到的密度，就不需要乘上gamma_rel
          fjx = fcx * part_q 
          fjy = fcy * part_q 
        ...
        IF (probes_for_species) THEN
          ...
          DO WHILE(ASSOCIATED(current_probe))
            ...
            IF (time > current_probe%probe_start_time) THEN
  ...
  SUBROUTINE push_photons(ispecies)
    ...
    DO WHILE(ASSOCIATED(current))
      ...
      !XY revise
      current_energy = c*SQRT(SUM(current%part_p**2))
       !IF (current_energy > 100*1e6*q0) THEN
       !     WRITE(*,*) 'en,cpe,p:', current_energy, current%particle_energy,current%part_p
      !IF (current%particle_energy < eps) THEN
      !ENDIF
